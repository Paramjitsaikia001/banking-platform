...
import { ArrowLeft, Send, CreditCard, Building2, Smartphone, Wallet, User, Phone, Banknote, Shield, Calendar } from "lucide-react";
...
export default function SendMoneyPage() {
  ...
  const paymentMethods = [
    { value: 'wallet', label: 'Wallet Transfer', icon: <Wallet className="h-4 w-4" /> },
  ...
  const handleSubmit = async (e?: React.FormEvent | null) => {
    ...
    if (paymentMethod === 'wallet' && !phoneNumber) {
      toast.error('Please enter phone number for wallet transfer');
      return;
    }
    ...
    // Check if user has sufficient balance for wallet transfers
    ...
    try {
      ...
      const transaction = {
        ...
        description: paymentMethod === 'wallet' 
          ? `Sent money via wallet to ${userName}` 
          : paymentMethod === 'bank_transfer'
          ? `Sent money via bank transfer (${selectedBank})`
          : paymentMethod === 'card_payment'
          ? `Sent money via card payment (${cardNumber.slice(-4)})`
          : `Sent money via ${paymentMethod}`,
        status: 'completed',
        recipientDetails: {
          upiId: paymentMethod === 'wallet' ? phoneNumber : recipientId
        },
        createdAt: new Date().toISOString()
      };

      // Update balance using shared context (only for wallet transfers)
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {paymentMethod === 'bank_transfer' && (
              <div className="space-y-4 p-4 border rounded-lg bg-muted/50">
                {/* Recipient Details */}
                <div className="space-y-2">
                  <Label htmlFor="recipientId">Recipient Account Number</Label>
                  <Input
                    id="recipientId"
                ...
                {/* Bank Account Selector for Source Account */}
                {!showManualBankForm && (
                  <BankAccountSelector
                    ...
                    description="Choose your linked bank account or enter details manually"
                  />
                )}
                ...
                {showManualBankForm && (
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <h3 className="text-lg font-medium">Enter Your Bank Details</h3>
                      <Button
                        type="button"
                        variant="outline"
                        size="sm"
                ...
                {!showOtpInput ? (
                  <Button
                    ...
                    disabled={
                      !recipientId || 
                      (selectedBankAccount ? !selectedBankAccount : (!selectedBank || !accountNumber || !ifscCode || !accountHolderName))
                    }
                    className="w-full"
                  >
                    Send OTP
            ...
            {paymentMethod === 'card_payment' && (
              <div className="space-y-4 p-4 border rounded-lg bg-muted/50">
                <div className="space-y-2">
                  <Label htmlFor="recipientId">Recipient UPI ID</Label>
                  <Input
                    id="recipientId"
